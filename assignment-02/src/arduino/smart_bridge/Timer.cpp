...
bool Timer::check_time_since(long time_from, long period){
  long time_now = millis(); // gets the time now in millis

  // checks if the amount of time that passed since time_from is greater than period
  if (time_now - time_from >= period){
    return true;
  }
  return false;
}